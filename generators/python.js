 ...
 * 变量列表的长度
 */
Blockly.Python.variableLength = 0

/**
 * 函数列表的长度
 */
Blockly.Python.funLength = 0
...
Blockly.Python.init = function(workspace) {
  ...
  //python 代码的尾部代码
  Blockly.Python.tailCodes = Object.create(null);
  ...
  if(Blockly.Python.variableLength !== variables.length){
    workspace.toolbox_.refreshSelection()
    Blockly.Python.variableLength = variables.length
  };
...
Blockly.Python.finish = function(code) {
  ...
  var tailCodes = []
  for(var name in Blockly.Python.tailCodes){
    tailCodes.push(Blockly.Python.tailCodes[name])
  }

  var ret = "# generated by Block\n";
  // imports
  if (imports.length !== 0) {
    ret += imports.join('\n') + "\n\n";
  }
  ...
  if(customFunctions.length!==Blockly.Python.funLength){
    mainWorkspace.toolbox_.refreshSelection()
    Blockly.Python.funLength = customFunctions.length
  }
  // libraries
